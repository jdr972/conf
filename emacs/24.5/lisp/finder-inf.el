...
(setq finder-keywords-hash
      #s(hash-table size 487 test eq rehash-size 1.5 rehash-threshold 0.8 data (abbrev (skeleton pcomplete emacs icomplete hippie-exp expand edmacro dabbrev completion autoarg) convenience (diff-mode compare-w keypad cua-base erc calc emacs table picture makeinfo flyspell winner windmove vcursor tmm ruler-mode repeat pcomplete minibuf-eldef mb-depth linum kmacro json imenu ido ibuffer hippie-exp frameset follow filesets filecache ffap ebuff-menu desktop delsel dabbrev completion chistory calculator bs autorevert autoinsert align) languages (erc smie emacs nxml sgml-mode xscheme vhdl-mode verilog-mode vera-mode tcl sql simula sh-script scheme ruby-mode python ps-mode perl-mode pascal opascal octave modula2 mixal-mode meta-mode m4-mode ld-script js idlwave icon fortran flymake f90 executable dcl-mode cwarn cperl-mode cfengine cc-mode cc-guess cap-words bat-mode autoconf asm-mode antlr-mode ada-mode tempo skeleton paren info-look allout align) lisp (inversion cedet unsafep trace testcover tabulated-list smie shadow re-builder pp emacs macroexp ewoc ert elp elint eieio eieio-core edebug debug checkdoc check-declare benchmark xscheme scheme inf-lisp hideshow strokes profiler isearchb color cmuscheme align) outlines (org hideshow hideif reveal outline foldout elide-head allout allout-widgets) wp (handwrite org nxml underline two-column tildify emacs table sgml-mode refill picture page-ext nroff-mode ispell enriched ebnf2ps whitespace ps-print printing allout) pgp (gnus epg epa emacs allout) gnupg (gnus epg epa emacs allout) comm (zeroconf webjump tramp tls telnet socks soap-client secrets rlogin rcirc net-utils ldap gnutls eudc dns dig dbus ange-ftp erc url bindat sgml-mode dns-mode sql talk shadowfile notifications master kermit ansi-color) processes (tramp erc url bindat eshell sql inf-lisp idlwave grep compile term shell server proced pcomplete emacs emacs-lock dirtrack cmuscheme ansi-color) terminals (tramp xt-mouse vt-control ansi-color) services (ansi-color) help (emacs woman novice man makesum informat info info-look icomplete help-at-pt finder ehelp) files (compare-w url po wdired view emacs shadowfile recentf locate ffap doc-view arc-mode) archives (arc-mode) msdog (arc-mode) editing (hideshow dcl-mode arc-mode) major-mode (dcl-mode arc-mode) extensions (calc gnus cookie1 tq tabulated-list rx ring regi regexp-opt emacs ewoc eldoc cl cl-lib benchmark avl-tree type-break tree-widget thingatpt tempo strokes skeleton ido emacs-lock electric ehelp array) emulations (viper edt cua-base emacs hl-line delsel autoarg) mouse (goto-addr browse-url artist xt-mouse emacs thingatpt t-mouse strokes msb mouse-drag mouse-copy ffap avoid) hardware (zeroconf dbus vt100-led emacs flow-ctrl battery) internal (emacs warnings syntax smie disass select novice loadhist gs delim-col) bookmarks (saveplace bookmark) placeholders (saveplace bookmark) annotations (bookmark) tools (semantic vc smerge-mode log-view emerge ediff diff cvs-status add-log reporter mail-utils trace re-builder package emacs ert elp eieio edebug debug copyright check-declare which-func make-mode hideshow gud grep glasses gdb-mi flymake etags ebrowse cpp compile bug-reference asm-mode time-stamp tempo speedbar skeleton sb-image pcomplete imenu format-spec find-file ezimage elide-head dframe calculator) i18n (gnus emacs po double descr-text) data (snmp-mode gnus url org bindat timeclock remember page-ext xml whitespace jka-compr emacs hexl hex-util cdl) faces (erc gnus enriched m4-mode ld-script cwarn cpp emacs paren hl-line hilit-chg hi-lock face-remap descr-text color) color (color) hex (color) rgb (htmlfontify color) hsv (color) hsl (color) cie-lab (color) background (color) mule (emacs ps-print) multilingual (emacs descr-text) character\ composition (emacs) expand (dabbrev) completion (crm dabbrev) unicode (emacs descr-text) file (dns-mode speedbar sb-image ezimage dframe) tags (ede semantic ebrowse speedbar sb-image ezimage dframe) dired (wdired emacs) window (emacs windmove follow) drag (emacs) drop (emacs) pdf (doc-view) ps (doc-view) dvi (doc-view) font (ps-print emacs) system-font (emacs) tool-bar-style (emacs) unix (emerge ediff diff telnet rlogin ispell sh-script make-mode gud gdb-mi executable cc-mode woman tar-mode sort proced emacs lpr locate find-lisp find-dired) mail (mh-e unrmail undigest uce supercite smtpmail sendmail rmail rfc822 rfc2368 reporter mspools metamail mail-utils mailclient mailalias mailabbrev hashcash footnote feedmail emacs mairix imap goto-addr gnus fortune time-date epa) message (feedmail gnus epa) face\ remapping (face-remap) display (hi-lock follow face-remap) user\ commands (face-remap) hypermedia (soap-client quickurl browse-url url org nxml sgml-mode css-mode emacs htmlfontify ffap) matching (regi re-builder thingatpt soundex misearch emacs hi-lock find-file ffap) filesets (filesets) c (hideshow hideif flymake cwarn cpp cmacexp cc-mode cc-guess emacs find-file) folding (foldout) minor-mode (hi-lock follow) frames (winner emacs talk hl-line) generic (emacs) comment (emacs) font-lock (emacs) colour (htmlfontify) html (shr shr-color eww gnus htmlfontify) markup (htmlfontify) etags (htmlfontify) buffer (msb ibuffer) multimedia (metamail gnus thumbs mpc emacs image-file image-dired iimage) docs (lisp-mnt checkdoc texnfo-upd texinfo texinfmt makeinfo info-xref) keyboard (keypad cua-base kmacro) md4 (md4) utilities (midnight) menu (erc msb) uncomment (emacs) desktop (notifications) notifications (notifications) password (secrets password-cache) cache (password-cache) passphrase (secrets password-cache) key (password-cache) print (handwrite ps-print printing) postscript (gnus handwrite ps-mode ebnf2ps ps-print printing) bdf (ps-print) multibyte (ps-print) vi (repeat) repeat (repeat) minibuffer (crm savehist emacs) scroll (scroll-all) crisp (scroll-all) brief (scroll-all) lock (scroll-all) spreadsheet (testcover ses) dijkstra (ses) gpm (t-mouse) linux (t-mouse) news (uudecode supercite metamail mail-utils footnote binhex newsticker netrc gnus time-date timezone) timers (type-break) virtual\ cursor (vcursor) environment (wdired) renaming (wdired) movement (windmove) xml (nxml xml) ada (ada-mode) project\ file (ada-mode) xref (ada-mode) antlr (antlr-mode) code\ generator (antlr-mode) awk (cc-mode) cc-mode (cc-mode) oop (cc-guess) perl (cperl-mode) dcl (dcl-mode) ebnf (ebnf2ps) c++ (hideshow ebrowse) fortran (org fortran f90) f90 (f90) java (conf-mode hideshow) comments (hideshow) blocks (hideshow) hiding (hideshow) javascript (js) g++ (mantemp) templates (mantemp) metafont (meta-mode) metapost (meta-mode) tex (org emacs tildify texnfo-upd texinfo texinfmt tex-mode reftex refbib bibtex bibtex-style meta-mode) knuth (mixal-mode) mix (mixal-mode) mixal (mixal-mode) asm (mixal-mode) mixvm (mixal-mode) the\ art\ of\ computer\ programming (mixal-mode) prolog (prolog) major (prolog) mode (prolog) sicstus (prolog) swi (prolog) mercury (prolog) ruby (ruby-mode) tcl (tcl) modes (tcl) vera (vera-mode) vhdl (vhdl-mode) mode-line (which-func) imenu (which-func) bib (refer refbib bib-mode) bibtex (bibtex) latex (emacs bibtex) conf (conf-mode) ini (conf-mode) windows (conf-mode) dns (dig dns-mode) master (dns-mode) zone (dns-mode) soa (dns-mode) memory (remember) todo (todo-mode remember) pim (remember) maint (reporter emacs gulp edebug debug copyright checkdoc check-declare texnfo-upd texinfo texinfmt) text (tildify) sgml (tildify) burma (emacs) cham (emacs) chinese (emacs) cyrillic (emacs) czech (emacs) multibyte\ character (emacs) character\ set (emacs) syntax (semantic mode-local emacs) category (emacs) ethiopic (emacs) european (emacs) greek (emacs) input\ method (emacs) korean (emacs) hanja (emacs) hebrew (emacs) indian (emacs) devanagari (emacs) japanese (emacs) khmer (emacs) lao (emacs) coding\ system (emacs) romanian (emacs) sinhala (emacs) slovak (emacs) tai\ viet (emacs) thai (emacs) word\ break (emacs) emacs (emacs) tibetan (emacs) vietnamese (emacs) calendar (gnus org todo-mode timeclock calendar icalendar diary-lib) holidays (calendar) util (time-date parse-time) relaxng (nxml) regexp (erc nxml) ccl (emacs) coding-system (emacs) fontset (emacs) isearch (emacs) iso (emacs) latin (emacs) kinsoku (emacs) charset (emacs) diagnosis (emacs) quail (emacs) tit (emacs) cxterm (emacs) normalization (emacs) data\ structures (avl-tree) avl (avl-tree) tree (cvs-status avl-tree) oo (inversion cedet eieio eieio-core chart) chart (chart) graph (chart) multiple\ elements (crm) debugging (elp) emacs-lisp (find-func) functions (find-func) variables (find-func) compiler (macroexp) macros (macroexp) strings (rx regexp-opt) regexps (rx regexp-opt) parsing (smie) indentation (smie) utility (unsafep testcover) safety (unsafep testcover) literate\ programming (org) reproducible\ research (org) r (org) statistics (org) comint (org) js (org) accounting (org) babel\ language (org) maxima (org) scheme (ede org) interactive\ shell (org) shen (org) org (gnus org) task (org) capture (org) org-mode (org) erc (org) irc (erc org) link (org) tables (org) plotting (org) emacsclient (org) markdown (org) url (vc erc url) vc (ede vc smerge-mode pcvs log-view log-edit emerge ediff diff diff-mode cvs-status compare-w add-log url) games (zone tetris studly spook solitaire snake pong mpuz morse life landmark hanoi gomoku gametree gamegrid fortune dunnet doctor dissociate decipher cookie1 bubbles blackbox animate 5x5) puzzles (5x5) utils (fortune) cursive\ writing (handwrite) neural\ network (landmark) adaptive\ search (landmark) chemotaxis (landmark) cancel-lock (gnus) hmac (hmac-md5 hmac-def gnus) sha1 (gnus) rfc2104 (hmac-md5 hmac-def gnus) web (gnus) icalendar (gnus) xpm (gnus) annotation (gnus) glyph (gnus) registry (gnus) synchronization (gnus) nntp (gnus) nnrss (gnus) network (network-stream net-utils dns gnus) external-body (gnus) partial (gnus) uudecode (uudecode gnus) binhex (binhex gnus) shar (gnus) forward (gnus) gnatsweb (gnus) gnus (gnus) mime (metamail gnus) s/mime (gnus) mml (gnus) searching (mairix gnus) ir (gnus) rss (newsticker gnus) hz (gnus) hz+ (gnus) smime (gnus) x\.509 (gnus) pem (gnus) openssl (gnus) spam (mh-e rmail gnus) gmane (gnus) report (gnus) bogofilter (gnus) bbdb (gnus) dspam (gnus) dig (dig gnus) whitelist (gnus) blacklist (gnus) hashcash (hashcash gnus) spamassassin (gnus) bsfilter (gnus) ifile (gnus) stat (gnus) crm114 (gnus) spamoracle (gnus) tls (tls gnutls gnus) ssl (tls gnutls gnus) gnutls (tls gnus) yenc (gnus) chat (erc) client (erc) internet (erc) button (erc) logging (erc) timestamp (erc) cua (cua-base) mark (cua-base) hypertext (browse-url) bind (dig) encryption (gnutls) mh-e (goto-addr) www (webjump goto-addr) hmac-md5 (hmac-md5) md5 (hmac-md5) keyed-md5 (hmac-md5) cram-md5 (sasl hmac-md5) atom (newsticker) ntlm (sasl ntlm) sasl (sasl ntlm) digest-md5 (sasl) soap (soap-client) web-services (soap-client) firewalls (socks) email (rmail feedmail) queue (feedmail) sendmail (feedmail) spray (feedmail) smtp (feedmail) draft (feedmail) history (mail-utils) filter (rmail) rmail (rmail) uce (uce) unsolicited\ commercial\ email (uce) pcl-cvs (pcvs log-edit cvs-status) cvs (smerge-mode pcvs log-edit cvs-status) status (cvs-status) patch (diff-mode) diff (diff-mode) comparing (ediff) merging (ediff) patching (ediff) commit (log-edit) log (log-edit) release\ management (pcvs) revision\ control (smerge-mode) merge (smerge-mode) diff3 (smerge-mode) conflict (smerge-mode) project (ede semantic) make (ede semantic) codegeneration (srecode))))

(provide 'finder-inf)
;; Local Variables:
